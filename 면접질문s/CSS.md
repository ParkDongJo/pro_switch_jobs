...
css 코드는 .css, .scss 같은 css 파일에 작성합니다. 하지만 이를 JS 파일로 가져와서 객체로 사용합니다. 이말은 JS 로직과 밀접하게 연결 된다는 것입니다.

덕분에 CSS의 Global namespace 로 인한 문제점이 해결됩니다. 컴포넌트와 함께 구조화 범위를 일치 시킬 수 있게 됩니다.

게다가 런타임 css-in-js 와 달리 빌드타임에 
