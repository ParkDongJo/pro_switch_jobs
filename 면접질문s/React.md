...
------
React v18 부터 점진적 hydration이 도입 되었습니다. 이 때 Suspens는 React 에게 컴포넌트가 읽어 드리고 있는 데이터가 아직 준비가 안되었음을 알려줄 수 있는 매커니즘 입니다.

Suspense 로 감싼 컴포넌트의 랜더링을 관련 비동기 작업이 끝날때까지 지연시키고, 그 동안 fallback에 정의된 컴포넌트를 보여줍니다. 그리고 비동기를 통한 데이터가 준비가 되면,
