...
----
#### 쿠키-세션 인증/인가 방식
쿠키와 세션으로 인증과 인가를 하는 방식이다. 둘은 상호 호환적인 기술이다. 다만 각 영역을 나눈다면,

쿠키는
- 서버쪽에서 생성된다. 클라이언트 쪽에서 관리한다.
- 인증 이후 매 요청 시 인가를 위해, 쿠키 정보를 서버로 전송한다.
세션은
- 서버쪽에서 생성하고 관리한다.
- 서버 메모리에 저장된다.
- sectionId를 발급해준다.
...
둘의 관계는
첫 인증 시 서버에서 sectionId를 발급받고, 이를 쿠키로 받는다.
이후 요청 시 인가를 위해, 쿠키 정보를 서버에 전송하고 그 안에는 sectionId 가 들어있다. 서버는 sectionId 를 찾아서 인가가 완료되면, 요청한 데이터를 내려다 준다.
